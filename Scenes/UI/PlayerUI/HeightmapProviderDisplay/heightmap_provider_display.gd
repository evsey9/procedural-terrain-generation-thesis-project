...
func _ready() -> void:
	...
	WorldGenerationSettings.Changed.connect(world_generation_settings_changed)
...
func setup_properties() -> void:
	...
	properties_box.end_group()
	properties_box.add_float("Preview Zoom", heightmap_provider.DisplayNoiseScale, 0.5, 100, 0.1)
...
func setup_noise_texture():
	noise_texture_rect.material.set_shader_parameter("noise_texture", heightmap_provider.NoiseTexture)
	noise_texture_rect.material.set_shader_parameter("noise_curve_texture", heightmap_provider.CurveTexture)
	noise_texture_rect.material.set_shader_parameter("contribution_curve_texture", heightmap_provider.ContributionCurveTexture)
	update_shader()
...
func _on_properties_box_number_changed(key: StringName, new_value: float) -> void:
	match key:
		...
		"Preview Zoom":
			heightmap_provider.DisplayNoiseScale = new_value
		_:
			pass
	properties_changed()
...
func update_shader():
	heightmap_provider.UpdateNoiseTexture()
	...
	noise_texture_rect.material.set_shader_parameter("sea_level_value", WorldGenerationSettings.SeaLevelRatio)
