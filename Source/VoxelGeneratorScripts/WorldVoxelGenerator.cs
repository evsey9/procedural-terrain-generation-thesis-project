...
public partial class WorldVoxelGenerator : VoxelGeneratorScript
{
	...
	{
		if (WorldGenerationSettings.Singleton is null || WorldHeightmapProvider.Singleton is null)
		{
			return;
		}

		worldGenerationSettings ??= WorldGenerationSettings.Singleton;
		...
		if (originInVoxels.Y > worldGenerationSettings.HeightmapMaxY)
		{
			outBuffer.Fill((Int32)Blocks.Air, (Int32)Channel);
		}
		else if (originInVoxels.Y + blockSize < worldGenerationSettings.HeightmapMinY)
		{
		...
		else
		{
			...
			for (Int32 z = 0; z < blockSize; z++)
			{
				...
				for (Int32 x = 0; x < blockSize; x++)
				{
					...
					// Water
					if (height < worldGenerationSettings.SeaLevelY && originY < worldGenerationSettings.SeaLevelY)
					{
						...
						if (originY + blockSize == worldGenerationSettings.SeaLevelY)
						{
	...
	{
		return worldHeightmapProvider?.GetHeightAt(x, z) ?? 0;
	}
}